...
/*content parse*/
require_once plugin_dir_path( __FILE__ ) . '/class-mdbp-parser-content.php';
...
if ( class_exists( 'WP_Importer' ) ) {
	...
	/**
	 * Class WP_Bricks_import for import theme demo content
	 */
	class WP_Bricks_import extends WP_Importer {
			...
			foreach ( $this->posts as $post ) {
				...
				if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
					...
					$postdata = array(
						...
						'post_content'   => preg_replace($pattern, $replacement, $post['post_content']),
		...
		function fetch_remote_file( $url, $post ) {
			...
			$remote_response = wp_remote_get( $url, array( //wp_safe_remote_get
					'timeout'  => 300,
					'stream'   => true,
...
//			return $parser->parse( $file );
			...
			$parser = new WPBricks_WXR_Parser();
