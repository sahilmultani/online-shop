...
const totalData = [];
wp.apiFetch( {
	path: 'client/block-count?data-type=template',
	method: 'POST',
} ).then( data => {
	totalData.push(data.count_templates)
} );
export default class Template_lib extends Component {
	constructor(props) {
		super(props);
		let tabnameSplit;
		if (this.props.data) {
	...
	loadFunc(page) {
		...
		wp.apiFetch( {
			path: 'client/template-data?cat=' + this.state.SearchCat + '&paged=' + pageNumber + '&s=' + this.state.searchData,
			method: 'POST',
		} ).then( data => {
			...
			<div className={"block_section"}>
				<div className="heading_section">
					<div className="wpbricks_title_section">
						<h2 className="tbl-getting-heading-library">{__('Template Library', 'wpbricks')}</h2>
					</div>
						...
						<form>
							<div className={"bricks-filter-opt"}>
								...
								<div className="bricks_search">
									...
									data-attr={this.state.tabName} 
									placeholder={__('Search Template', 'wpbricks')}
												...
												<div className="inner">
													...
													<div className="content">
														...
														<div className="description">
															<p>{__("Don't find a template you would like to Import?", 'wpbricks')}<br></br><a target="_blank" href="https://www.thedotstore.com/suggest-a-feature/">{__('Submit', 'wpbricks')}</a>{__(' your desire Template suggestion!', 'wpbricks')}</p>
															<div className="back-to-layout-button"><span className="button wpbricks-sites-back" onClick={this.backToTemplate}>{__('Back to Templates', 'wpbricks')}</span></div>
														</div>
													</div>
												</div>
											</div>
